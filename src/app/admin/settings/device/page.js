  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Printer selection, receipt customization, and timezone. Managers can edit this area.</p>
      </div>

      <form action={saveDeviceSettings} className="grid gap-6 xl:grid-cols-[360px,1fr]">
        <div className="space-y-6">
          <Card className="p-5">
            <h3 className="text-sm font-bold uppercase tracking-wide text-slate-500">Select Printer</h3>
            <select name="printerTarget" defaultValue={settings?.printers?.[0]?.printerTarget || ""} disabled={!canEdit} className="mt-4 w-full rounded-xl border border-slate-200 px-4 py-3">
              {settings?.printers?.map((printer) => (
                <option key={printer.id} value={printer.printerTarget || printer.name}>{printer.name} ({printer.printerConnection || "Unknown"})</option>
              ))}
            </select>
          </Card>

          <Card className="p-5 space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wide text-slate-500">Timezone</h3>
          ...
          <Card className="p-5 space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wide text-slate-500">Receipt Controls</h3>
            ...
            <div className="grid gap-3 text-sm text-slate-600">
              <label><input name="receiptShowLogo" type="checkbox" defaultChecked={settings?.receiptShowLogo} disabled={!canEdit} className="mr-2" />Show Logo</label>
              <label><input name="receiptShowSeller" type="checkbox" defaultChecked={settings?.receiptShowSeller} disabled={!canEdit} className="mr-2" />Show Seller Details</label>
              <label><input name="receiptShowBuyer" type="checkbox" defaultChecked={settings?.receiptShowBuyer} disabled={!canEdit} className="mr-2" />Show Buyer Details</label>
              <label><input name="receiptShowQr" type="checkbox" defaultChecked={settings?.receiptShowQr} disabled={!canEdit} className="mr-2" />Show QR</label>
              <label><input name="receiptShowSign" type="checkbox" defaultChecked={settings?.receiptShowSign} disabled={!canEdit} className="mr-2" />Show Signature Area</label>
            </div>
            {canEdit && <Button type="submit">Save Device Settings</Button>}
          </Card>
        </div>
        ...
        <Card className="p-8">
          ...
          <div className="mt-6 max-w-lg rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8">
            <div className="text-center text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">{settings?.receiptTheme || "modern"} theme</div>
            <div className="mt-6 text-center text-xl font-black text-slate-900">BPC Dhaka</div>
            <div className="mt-2 text-center text-sm text-slate-500">Receipt #INV-20260409-0001</div>
            <div className="mt-6 space-y-3 text-sm text-slate-700">
              ...
              <div className="border-t border-slate-200 pt-3 font-bold"><div className="flex justify-between"><span>Total</span><span>640.00</span></div></div>
            </div>
          </div>
        </Card>
      </form>
