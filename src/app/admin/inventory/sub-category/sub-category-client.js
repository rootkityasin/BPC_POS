...
export function SubCategoryClient({ categories, subCategories }) {
  const [activeTab, setActiveTab] = useState("subCategories");
  const [searchQuery, setSearchQuery] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [formData, setFormData] = useState({ nameEn: "", nameBn: "", categoryId: "" });
  ...
  return (
    <div className="flex flex-col bg-[#fdfdfd] min-h-screen">
      <div className="mb-6 mt-2">
        <h2 className="text-[28px] font-bold text-[#ff242d]">Sub-Category</h2>
      </div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button 
          onClick={() => { setIsAddModalOpen(true); setFormData({ nameEn: "", nameBn: "", categoryId: categories[0]?.id || "" }); }}
      ...
      <div className="rounded-[24px] bg-white shadow-[0_15px_40px_rgba(0,0,0,0.03)] border border-slate-50 p-8">
              ...
              {activeTab === "subCategories" ? (
                ...
                <>
                  <div className="col-span-3 flex items-center gap-4">
                    <div
                      className="h-2 w-2 rounded-full flex-shrink-0"
                      style={{ backgroundColor: item.color || "#ff242d" }}
                    />
                    <span className="font-bold text-[14px] text-[#ff242d]">{item.nameEn}</span>
                  </div>
                  <div className="col-span-3 text-[14px] text-[#ff242d]/70 font-medium">
        ...
        <div className="mt-12 flex items-center justify-between border-t border-slate-100 pt-6">
          <div className="text-[13px] font-medium text-slate-500">
            Showing {filteredData.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} categories
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[32px] bg-white p-8 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-[#ff242d]">Add New Sub-Category</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700">Sub-Category Name (English)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700">Sub-Category Name (Bengali)</label>
                <input 
                  type="text" 
                  value={formData.nameBn}
                  onChange={(e) => setFormData({ ...formData, nameBn: e.target.value })}
                  className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-[#ff242d]" 
                  placeholder="e.g. মুরগি" 
                />
              </div>
              <div>
                ...
                <select 
                  ...
                  className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-[#ff242d] bg-white"
                >
                  {categories.map(cat => (
              ...
              <div className="mt-8 flex gap-3">
                ...
                <button 
                  ...
                  onClick={async () => {
                    try {
                      const res = await fetch('/api/v1/subcategory', {
                        method: 'POST',
